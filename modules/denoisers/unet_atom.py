...
class UNetAtom(nn.Module):
    def __init__(
        ...
        self.first_conv_x = nn.Conv2d(
            in_channel * 2, inner_channel, kernel_size=3, padding=1
        )
        ...
        self.num_resolutions = num_mults - 1
        pre_channel = inner_channel
        feat_channels = [pre_channel]
        downs = [nn.Conv2d(inner_channel, inner_channel, kernel_size=3, padding=1)]
    ...
    def check_image_size(self, x):
        _, _, h, w = x.size()
        padder_size = 2**self.num_resolutions
    ...
    def forward(self, x, time, cond):
        ...
        x, h0, w0 = self.check_image_size(x)

        x = self.first_conv_x(x)
        ...
        for layer in self.ups:
            if isinstance(layer, ResnetBlocWithAttn):
                h, w = feats[-1].shape[2:]
                x = layer(torch.cat((x[:, :, :h, :w], feats.pop()), dim=1), t)
            else:
                x = layer(x)
