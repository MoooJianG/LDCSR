...
class S2Model(DDPM):
    ...
    def forward(self, x, c, scale, osize, *args, **kwargs):
        ...
        # 固定从最大的t开始
        t = torch.full(
            (x.shape[0],), self.num_timesteps - 1, device=self.device, dtype=torch.long
        )
    ...
    def p_losses(self, x_start, cond, t, scale, osize, noise=None, *args, **kwargs):
        ...
        for i in reversed(range(0, self.num_timesteps)):
            ts = torch.full(
                (x_start.shape[0],), i, device=self.device, dtype=torch.long
            )
            x_hat = self.p_sample(
                x_hat,
                cond_dict,
        ...
        # simple loss
        loss_simple = self.get_loss(x_hat, x_start, mean=False).mean([1, 2, 3])
    ...
    @torch.no_grad()
    def sample_log(self, cond, batch_size, shape, **kwargs):
