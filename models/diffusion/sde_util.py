...
class SDE(abc.ABC):
    ...
    def forward(self, x0, T=-1):
        ...
        x = x0.clone()
        for t in tqdm(range(1, T + 1), total=T):
...
class IRSDE(SDE):
    ...
    def _initialize(self, max_sigma, T, schedule, eps=0.01):
        ...
        def get_sigma_bars(thetas_cumsum):
            return torch.sqrt(
                max_sigma**2 * (1 - torch.exp(-2 * thetas_cumsum * self.dt))
            )
        ...
        thetas_cumsum = (
            get_thetas_cumsum(thetas) - thetas[0]
        )  # for that thetas[0] is not 0
    ...
    def ode_reverse_drift(self, x, score, t):
        return (
            self.thetas[t] * (self.mu - x) - 0.5 * self.sigmas[t] ** 2 * score
        ) * self.dt

    def dispersion(self, x, t):
        return self.sigmas[t] * (torch.randn_like(x) * math.sqrt(self.dt)).to(
            self.device
        )
        ...
        for t in tqdm(reversed(range(1, T + 1)), total=T):
            ...
            if save_states:  # only consider to save 100 images
                ...
                if t % interval == 0:
                    ...
                    os.makedirs(save_dir, exist_ok=True)
                    tvutils.save_image(
                        x.data, f"{save_dir}/state_{idx}.png", normalize=False
                    )
...
class DenoisingSDE(SDE):
        ...
        for t in tqdm(reversed(range(1, T + 1)), total=T):
            ...
            if save_states:
                ...
                if t % interval == 0:
                    ...
                    idx = t // interval
                    tvutils.save_image(
                        state.data, f"{save_dir}/state_{idx}.png", normalize=False
                    )
    ...
    def get_optimal_timestep(self, sigma, eps=1e-6):
        ...
        thetas_cumsum_hat = (
            -1 / (2 * self.dt) * math.log(1 - sigma**2 / self.max_sigma**2 + eps)
        )
