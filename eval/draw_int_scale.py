...
offset = np.array([[0, 1], [0.5, 1]])

# Set dataset (selected examples, please uncomment as needed)
# =====
setName = "AID"
degrade = "x4.0"  # scale factor
img_name = "center_4.png"
zoom_p = np.array([[60, 120], [500, 300]])
zoom_size = [100, 100]
# =====
setName = "DOTA"
degrade = "x2.0"  # scale factor
img_name = "P0798.png"
zoom_p = np.array([[150, 200], [350, 120]])
zoom_size = [100, 100]
# =====
setName = "DIOR"
...
def draw_box_zoom(img, zoom_p, zoom_size, offset):
    ...
    rimg = Image.new(
        "RGB", (ori_size[0] + int(ori_size[1] * 0.5), ori_size[1]), "white"
    )
    rimg.paste(img, (0, 0))
...
fig, axes = plt.subplots(nrows=rows, ncols=cols, figsize=(18, 4.8))
plt.subplots_adjust(wspace=0.02, hspace=-0.2)  # 缩小子图之间的间距
plt.subplots_adjust(left=0, bottom=-0.03, top=1.11, right=1)  # 去除图像四周的空白
for i, method in enumerate(method_name):
    ...
    ax.text(0.5, -0.05, title_text, fontsize=fontsize+2, ha='center', va='top', transform=ax.transAxes)
    if i == 0:
        ax.text(
            0.03, 0.97, f"{degrade}", 
            fontsize=16,
        ...
        # ax.text(0.02, 0.98, f"{degrade}", fontsize=fontsize, ha='left', va='top', transform=ax.transAxes)
    ax.axis("off")
