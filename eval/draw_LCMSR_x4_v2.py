...
fig, axes = plt.subplots(nrows=rows, ncols=cols, figsize=(12, 1.8))
plt.subplots_adjust(wspace=0.05, hspace=-0.25)  # 缩小子图之间的间距
plt.subplots_adjust(left=0, bottom=-0.4, top=1.59, right=1)  # 去除图像四周的空白
...
# plt.tight_layout()
plt.savefig(
    os.path.join(savepath, f'LCMSRv2_{setName}_{degrade.replace(".", "")}.png'),
    dpi=200,
)
