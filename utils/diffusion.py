...
class Diffusion(nn.Module):
    ...
    def p_mean_variance(self, model, x, t, clip_denoised, return_pred_x0, **kwargs):
        if self.t_encode_mode == "discrete":
            ...
            #     .to(x.device)
            # )
            t_noise_level = extract(self.sqrt_alphas_cumprod, t, x.shape)
        ...
        **kwargs,
        ...
        with torch.no_grad():
            for i, t_noise_level in enumerate(reversed(Gamma_bar)):
                ...
                pred_x0 = model(x, t_noise_level, **kwargs)
...
def samples_fn(model, diffusion, cond, shape):
    samples = diffusion.p_sample_loop(
        model=model, cond=cond, shape=shape, noise_fn=torch.randn
    )
    return {"samples": samples}


def progressive_samples_fn(
    model, diffusion, cond, shape, device, include_x0_pred_freq=50
):
...
def fast_samples_fn(
    model, diffusion, cond, shape, device, skip=10, eta=0.0, include_x0_pred_freq=10
):
